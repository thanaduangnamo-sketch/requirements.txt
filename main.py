...
async def setup_saveroles_command(interaction: discord.Interaction, ลิงก์รูปภาพ: str = "https://i.pinimg.com/736x/14/68/59/146859926bd33323535af3b8697b024d.jpg"):
    ...
    await interaction.response.send_message("✅ สร้างหน้าต่างระบบเซฟและคืนยศเรียบร้อยแล้วครับ", ephemeral=True)
...
class TokenModal(discord.ui.Modal, title="AEGIS : TOKEN CHECKER"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        result_embed.set_footer(text="AEGIS BOT / SHOP — TOKEN CHECKER")
...
async def ticket_command(interaction: discord.Interaction, รูปภาพ: discord.Attachment = None, ลิงก์รูปภาพ: str = None):
    ...
    await interaction.response.send_message("✅ ส่งหน้าต่างติดต่อแอดมินเรียบร้อยแล้วครับ", ephemeral=True)
