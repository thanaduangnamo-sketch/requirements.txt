...
class VerifyModal(discord.ui.Modal, title="🔐 ยืนยันตัวตน"):
    def __init__(self, correct_code: str, role_id: int):
        ...
        self.code_input = discord.ui.TextInput(
            ...
            placeholder="กรอกรหัสตัวเลข 4 หลัก...",
            min_length=4,
            max_length=4,
            required=True
        )
        ...
        if user_code == self.correct_code:
            ...
            if role:
                try:
                    ...
                    await interaction.response.send_message("❌ บอทมียศต่ำกว่ายศที่ต้องมอบ", ephemeral=True)
            else:
                await interaction.response.send_message("❌ ไม่พบยศในระบบ", ephemeral=True)
        else:
            embed = discord.Embed(
                title="❌ รหัสผ่านไม่ถูกต้อง",
                description="กรุณากดปุ่มเพื่อรับรหัสผ่านใหม่",
...
class PersistentVerifyView(discord.ui.View):
    ...
    @discord.ui.button(label="ยืนยันตัวตน", style=discord.ButtonStyle.success, emoji="🛡️", custom_id="persistent_verify_button")
...
@bot.tree.command(name="setup-verify", description="🛡️ สร้างกล่องยืนยันตัวตน")
@app_commands.describe(
    role="เลือกยศที่จะแจก",
    title="หัวข้อประกาศ",
...
async def setup_verify(
    ...
    if not description:
        description = "กรุณากดปุ่ม **ยืนยันตัวตน** ด้านล่างเพื่อรับรหัสผ่าน 4 หลัก"
    ...
    embed.set_footer(text="Verification System", icon_url=bot.user.display_avatar.url)

    view = PersistentVerifyView(role_id=role.id)
    
    await interaction.channel.send(embed=embed, view=view)
    await interaction.response.send_message("✅ สร้างระบบยืนยันตัวตนเรียบร้อย", ephemeral=True)
