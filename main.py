...
class VerifyModal(discord.ui.Modal, title="🔒 กรอกรหัสเพื่อยืนยันตัวตน"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_code == self.correct_code:
            # ดึงยศที่ตั้งไว้จากข้อความ Embed โดยตรง เพื่อไม่ต้องล็อก Role ID
            embed = interaction.message.embeds[0]
            role_mention = embed.description.split("🎁 **ยศที่จะได้รับ:** ")[1].strip()
...
# ------------------------------------------
# 🔘 View (ปุ่มกดแบบ Persistent อยู่ถาวรแม้รีบอท)
# ------------------------------------------
class VerifyView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None) # timeout=None สำคัญมากในการทำให้ปุ่มอยู่ได้ตลอดไป

    # custom_id ล็อกชื่อไว้เพื่อให้บอทจำปุ่มเดิมได้ตอนรีสตาร์ท
    @discord.ui.button(label="ยืนยันตัวตน (Verify)", style=discord.ButtonStyle.success, emoji="🛡️", custom_id="persistent_verify_button")
    async def verify_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        generated_code = str(random.randint(1000, 9999))
        modal = VerifyModal(correct_code=generated_code)
        await interaction.response.send_modal(modal)

# ==========================================
# ⚙️ 3. Slash Commands & Event
# ==========================================
@bot.event
async def on_ready():
    # 🔴 จุดสำคัญ: ผูก View เข้ากับบอทตั้งแต่ตอนเริ่มทำงาน เพื่อให้ปุ่มเก่าใช้ได้เสมอแม็หลังรีสตาร์ท
...
async def setup_verify(
    ...
    # เรียกใช้ VerifyView ที่เป็นปุ่มถาวร
    view = VerifyView()
